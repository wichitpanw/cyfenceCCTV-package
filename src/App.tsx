...
import SurveyReportPreviewModal from "./components/SurveyReportPreviewModal";
...
export default function App() {
  ...
  const [previewProject, setPreviewProject] = useState<ProjectSurvey | null>(null);
  ...
  const handlePrintProjectPDF = (proj: ProjectSurvey, autoPrint: boolean = true) => {
    ...
    if (printWin) {
      ...
      const pricingItems = proj.pricingItems || [];
      const discount = proj.discount || 0;
      const vatRate = proj.vatRate || 0;
...
<head>
  <meta charset="UTF-8">
  <title>เอกสารสำรวจกล้อง - ${customerInfo?.customerName || "-"}</title>
...
<body>
  ...
  <div class="info-grid">
    <div class="info-box">
      ...
      <div class="info-row">ชื่อหน่วยงาน: <span>${customerInfo?.customerName || "-"}</span></div>
...
<script>
  ...
  // Print Detailed Survey Report (PowerPoint Slide Preview)
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 flex flex-col font-sans" id="applet-primary-layout">
      ...
      {/* PowerPoint Survey Report Widescreen Preview Modal */}
      {previewProject && (
        <SurveyReportPreviewModal
