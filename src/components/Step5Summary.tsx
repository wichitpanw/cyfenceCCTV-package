      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-zinc-50 p-5 rounded-2xl border border-zinc-200 space-y-3">
          <div className="flex items-center gap-2 border-b border-zinc-200 pb-2">
            ...
            <h4 className="font-semibold text-xs text-zinc-650 uppercase tracking-wide font-sans">รายละเอียดลูกค้า / สถานที่</h4>
          </div>
          <div className="text-xs space-y-2">
            <div>
              <span className="text-zinc-500 block">ลูกค้า/บริษัท:</span>
              <strong className="text-zinc-900 text-sm">{customerInfo.customerName || "-"}</strong>
            </div>
            <div>
              <span className="text-zinc-500 block">โครงการ:</span>
              <span className="text-zinc-800 font-semibold">{customerInfo.projectName || "-"}</span>
            </div>
            {customerInfo.address && (
              <div>
                <span className="text-zinc-500 block">สถานที่ติดตั้ง:</span>
                <p className="text-zinc-600 leading-normal line-clamp-2">{customerInfo.address}</p>
              </div>
            )}
            {(customerInfo.latitude || customerInfo.longitude) && (
              <div className="flex items-center gap-1 bg-zinc-100 px-2.5 py-1 rounded-lg w-fit border border-zinc-200">
                <MapPin className="w-3 h-3 text-zinc-600" />
                <span className="font-mono text-[10px] text-zinc-700">{customerInfo.latitude}, {customerInfo.longitude}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-zinc-50 p-5 rounded-2xl border border-zinc-200 space-y-3">
          <div className="flex items-center gap-2 border-b border-zinc-200 pb-2">
            ...
            <h4 className="font-semibold text-xs text-zinc-650 uppercase tracking-wide font-sans">สเปกความต้องการหลัก</h4>
          </div>
          <div className="text-xs space-y-2">
            <div className="flex justify-between items-center text-xs py-1.5 bg-zinc-150 px-2.5 rounded-lg border border-zinc-250">
              <span className="text-zinc-700 font-medium">การติดตั้งกล้องทั้งหมด:</span>
              <strong className="text-[#0071e3] font-mono text-base">{totalCamerasCount} ตัว</strong>
            </div>
            <div className="grid grid-cols-2 gap-2 mt-1">
              <div>
                <span className="text-zinc-500 block">แบรนด์กล้อง:</span>
                <span className="font-semibold text-zinc-850">{requirements.cameraBrand || "ไม่ระบุ"}</span>
              </div>
              <div>
                <span className="text-zinc-550 block">เครื่องบันทึก NVR:</span>
            ...
            <div>
              <span className="text-zinc-500 block flex items-center gap-1">
                <HardDrive className="w-3.5 h-3.5 text-zinc-550" /> แพคเกจเก็บข้อมูล:
              </span>
              <span className="font-medium text-zinc-800">{requirements.storagePackage || "-"}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-zinc-50 p-5 rounded-2xl border border-zinc-200 space-y-3">
          <div className="flex items-center gap-2 border-b border-zinc-200 pb-2">
            ...
            <h4 className="font-semibold text-xs text-zinc-650 uppercase tracking-wide font-sans">รายงานการสำรวจ (Survey)</h4>
          </div>
          <div className="text-xs space-y-2">
            {hasSurveyReport ? (
              <>
                <div className="p-2.5 bg-zinc-100 border border-zinc-300 rounded-xl">
                ...
                <div className="grid grid-cols-2 gap-2 text-[10px] text-zinc-500 mt-1">
                  <div>• เสาเหล็กเสริม: <strong className="text-zinc-800">{poleCount} ต้น</strong></div>
                  <div>• แขน Support ยึด: <strong className="text-zinc-800">{supportArmCount} ชุด</strong></div>
                </div>
              </>
            ) : (
              <div className="p-3 bg-zinc-100 border border-zinc-250 rounded-xl space-y-1">
            ...
            <tbody className="divide-y divide-zinc-200">
              {pricingItems.map((item, idx) => (
                <tr key={item.id} className="hover:bg-zinc-50/50">
                  ...
                  <td className="py-2.5 px-3 font-semibold text-zinc-800 whitespace-normal break-words">
                    {item.name}
                  </td>
                  <td className="py-2.5 px-3 text-center">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-medium uppercase whitespace-nowrap ${
                      item.category === "hardware" ? "bg-zinc-100 text-zinc-800 border border-zinc-250" :
                      item.category === "accessory" ? "bg-zinc-100 text-zinc-600 border border-zinc-200" :
                  ...
                  <td className="py-2.5 px-3 text-center text-zinc-455">{item.unit}</td>
                  <td className="py-2.5 px-3 text-zinc-500 text-[10px] leading-relaxed whitespace-normal break-words">
              ...
              {cameraPoints.map((pt, ind) => {
                ...
                return (
                  ...
                  >
                    ...
                    <strong className="block text-zinc-800 truncate text-xs">{pt.name}</strong>
